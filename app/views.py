...
def generate_pdf(reg_course, student, session, semester, confirmReg):
    class PDF(FPDF, HTMLMixin):
        def header(self):
            ...
            # Title
            self.cell(
                170, 0, "ACHIEVERS UNIVERSITY, OWO", border=False, ln=1, align="C"
            )
            # line break
            self.ln(1)
            ...
            self.cell(
                170,
                7,
                "Owo, Ondo State, Nigeria Website: www.achievers.edu.ng",
                border=False,
                ln=1,
                align="C",
            )
            ...
            # Title
            self.cell(
                170, 5, f"COURSE REGISTRATION FORM", border=False, ln=1, align="C"
            )
            self.ln(1)
            # logo
    ...
    if request.method == "POST":
        ...
        if is_registered:
            return render(
                ...
                {
                    "student": student,
                    "sess": current_academic_session,
                    "semes": current_academic_semester,
                    "exist": "true",
                    "semester_passed": semester,
                    "session_passed": curr_session,
                },
            )
        # student = get_object_or_404(Student, user=user)
        ...
        courses = Course.objects.filter(
            level=level, programmes=student.programme, semester=semester
        )
    ...
    if request.method == "GET" and "sess" in request.GET:
        ...
        return render(
            ...
            "printCopy.html",
            {
                ...
                "student": student,
                "session_passed": sess,
                "semester_passed": semes,
            },
        )

    return render(
        request, "reg.html", {"student": student, "sess": "2024/2025", "semes": "first"}
    )
...
def courseMain(request):
    ...
    if request.method == "POST":
        ...
        for id in courses:
            ...
            #     )
            #     course_exist.save()
        confirmReg = confirmRegister.objects.create(
            student=student,
            session=get_object_or_404(Session, year=sess),
            semester=get_object_or_404(Semester, name=semes),
            level=get_object_or_404(Level, name=student.currentLevel),
...
def changePassword(request):
    if request.method == "POST":
        ...
        if new_password != confirm_password:
            return render(
                request, "changepassword.html", {"error": "Use same password"}
            )
...
def adminCourseManagement(request):
    if request.user.is_authenticated:
        ...
        if request.method == "POST":
            ...
            # programmes = request.POST['programmes']
